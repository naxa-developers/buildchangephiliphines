...
class Category extends React.Component {
  ...
  onSiteTapped(subStepId) {
    Actions.CheckList({
      title: strings.title_checklist,
      item: this.props.item.new_checklists.filter(
        element => subStepId === element.sub_checklists[0].substep
      )
    });
  }
  ...
  render() {
    ...
    return (
      <ListView
        ...
        renderRow={rowData => (
          <TouchableOpacity onPress={this.onSiteTapped.bind(this, rowData.id)}>
            <View>
              <CardSection>
                <Circle
                ...
                <View style={cointainerStyle}>
                  <Text style={titleStyle}>
                    {getLocalizedText(rowData.local_title, rowData.title)}
                  </Text>
